...
Return _the result table ordered by  _the result ordered by**level** in **ascending** order, then by **budget** in **descending** order, and finally by **employee_name** in **ascending** order_._
...
WITH RECURSIVE EmployeeHierarchy AS (
    ...
    SELECT 
        ...
        salary,
        1 AS level,
        CAST(employee_id AS CHAR(255)) AS path
    FROM Employees
...
SELECT 
    ...
    eh1.employee_name,
    eh1.level,
    COUNT(DISTINCT eh2.employee_id) - 1 AS team_size,
    SUM(eh2.salary) AS budget
FROM EmployeeHierarchy eh1
LEFT JOIN EmployeeHierarchy eh2 
    ON (eh2.path LIKE CONCAT(eh1.path, ',%') OR eh2.path = eh1.path)
GROUP BY eh1.employee_id, eh1.employee_name, eh1.level
ORDER BY eh1.level ASC, budget DESC, eh1.employee_name ASC;
```
